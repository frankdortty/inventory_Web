...
$query = "SELECT id, customer_name, created_at, total FROM sales WHERE company = '$companyName' ORDER BY created_at DESC";
...
$queryCurrent = "SELECT SUM(total) AS total FROM sales WHERE company = '$companyName' AND DATE_FORMAT(created_at, '%Y-%m') = '$currentMonth'";
$resultCurrent = mysqli_query($conn, $queryCurrent);
if ($resultCurrent && $row = mysqli_fetch_assoc($resultCurrent)) {
    $totalExpense = $row['total'] !== null ? $row['total'] : 0;
} else {
    $totalExpense = 0;
}
...
$queryPrev = "SELECT SUM(total) AS total FROM sales WHERE company = '$companyName' AND DATE_FORMAT(created_at, '%Y-%m') = '$previousMonth'";
$resultPrev = mysqli_query($conn, $queryPrev);
if ($resultPrev && $rowPrev = mysqli_fetch_assoc($resultPrev)) {
    $previousMonthExpense = $rowPrev['total'] !== null ? $rowPrev['total'] : 0;
} else {
    $previousMonthExpense = 0;
}
...
<div class="container">
    ...
    <div class="search-filter-container">
        <div class="search-container">
            ...
            <input type="text" class="search-bar" id="search" placeholder="Search by customer name">
        </div>
    ...
    <div class="expense-list" id="expenseList">
        <?php foreach ($expenses as $expense): ?>
            <div class="expense-item" 
                ...
                data-name="<?php echo strtolower($expense['customer_name']); ?>" 
                data-date="<?php echo $expense['created_at']; ?>" 
                onclick="goToExpenseDetails(<?php echo $expense['id']; ?>)">
                <div>
                    <div class="expense-name"><?php echo htmlspecialchars($expense['customer_name']); ?></div>
                    <div class="expense-time"><?php echo htmlspecialchars($expense['created_at']); ?></div>
                </div>
                <div class="expense-amount">$<?php echo number_format($expense['total'], 2); ?></div>
            </div>
        <?php endforeach; ?>
    </div>
</div>
